...
const experiencesData = [
  {
    ...
    description: [
      { title: "プロジェクト概要", content: "SUWARPGプロジェクトは、諏訪市の観光資源を活用したご当地RPGを開発・運用し、地域の魅力を発信することを目的とした取り組みです。現在もゲームの開発が進行中であり、ストーリーやキャラクターの設計、ゲームイベントの実装、マップ開発などを中心に行っています。特にマップ開発については私自身がほとんど任されており諏訪市の再現を行っています。" },
      { title: "使用技術", content: "JavaScript,RPGツクールMZ" },
      { title: "成果", content: "これまでに、諏訪市役所でのプレゼンテーションを通じて、プロジェクトへの協力を得ることに成功しました。この協力により、地域資源を活用したゲーム内容を展開する基盤を築くことができました。プレゼンテーションの際の様子を捉えた画像を添付しています。また地元企業にも協力をしてもらい実際にカヤックの体験などをさせていただきゲーム内でカヤック体験ができる場所としてマップを作成させていただきました。" },
      { title: "今後の展望", content: "今後の展望としては開発をある程度進めてプロトタイプ完成を目指すことです。まだマップ化できていな箇所も多くそれらの作成やそのマップでのイベント作成など日々開発を進めています。"},
    ],
  ...
  },
  {
    ...
    description: [
      ...
      {title: "苦労した点",content: "商品を企画する際にどのような商品を顧客に届けたいかをブランドコンセプトに合うように作成するのが困難だった。また顧客はどんな服が欲しいのかなどトレンドが変わるのが早い中でいかに自身のブランドの芯を保ちつつ顧客に価値を提供できるかを考えることに時間を費やした。"},
      {title: "成果",content:"現在では少しずつだが無名のブランドから売れるようになり始めた。顧客に価値を届けられたときは飛び跳ねた記憶がある。またブランドを紹介したいという人まで現れていて続けてよかったと感じた。"},
      {title: "今後の展望",content: "今後はさらなる顧客獲得に向けて戦略を立てて価値を提供していきたい。廃れないよう日々トレンドをアップデートしていきさらなる価値を提供できるようにしたい。"}
    ],
    link: "https://urbansophist.base.shop/",
  }
];
